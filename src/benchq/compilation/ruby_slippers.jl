...
function run_ruby_slippers(
    ...
    if verbose
        ...
        (lco, adj, input_nodes, output_nodes) = @time get_graph_state_data(circuit, true, max_graph_size, hyperparams, max_time)
    else
        (lco, adj, input_nodes, output_nodes) = get_graph_state_data(circuit, false, max_graph_size, hyperparams, max_time)
    end
    return pylist(lco), python_adjlist!(adj), pylist(input_nodes), pylist(output_nodes)
end
...
function get_graph_state_data(
    ...
    return lco, adj, input_nodes, output_nodes
end
...
@inline function remove_edge!(adj, vertex_1, vertex_2)
    remove!(adj[vertex_1], vertex_2)
    remove!(adj[vertex_2], vertex_1)
end
