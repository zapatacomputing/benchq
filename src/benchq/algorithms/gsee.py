...
from ..data_structures import AlgorithmDescription, ErrorBudget
...
def qpe_gsee_algorithm(hamiltonian, precision, failure_tolerance):
    ...
    error_budget = ErrorBudget(failure_tolerance)
    n_calls = np.ceil(np.log(1 / error_budget.circuit_generation_failure_tolerance))
    return AlgorithmDescription(program, error_budget, n_calls)
